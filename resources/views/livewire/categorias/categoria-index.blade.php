<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Gestiona todas tus categorías') }}</flux:subheading>
                ...
                <tbody>
                    @foreach ($categorias as $categoria)
                        <tr
                            ...
                            <td class="px-6 py-2 font-medium text-gray-900 dark:text-white">{{ $categoria->id }}</td>
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $categoria->nombre }}</td>
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $categoria->descripcion }}</td>
                            <td class="px-6 py-2">
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @can('categoria.delete')
                                        <button wire:click="delete({{ $categoria->id }})" wire:confirm="¿Estás seguro que deseas eliminar esta categoria?"
