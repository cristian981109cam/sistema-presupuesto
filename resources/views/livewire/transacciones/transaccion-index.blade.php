<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Transacciones') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Gestiona todas tus transacciones') }}</flux:subheading>
                ...
                <tbody>
                    @foreach ($transacciones as $transaccion)
                        <tr
                            ...
                            <td class="px-6 py-2 font-medium text-gray-900 dark:text-white">{{ $transaccion->id }}</td>
                            ...
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">
                                @if($transaccion->ingresoGasto)
                                    {{ $transaccion->ingresoGasto->tipo }}
                                @else
                                    <span class="text-sm text-gray-500">—</span>
                                @endif
                            </td>
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ number_format($transaccion->monto, 2) }}</td>
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $transaccion->nota }}</td>
                            <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $transaccion->fecha }}</td>
                            <td class="px-6 py-2">
                                <div class="flex flex-wrap gap-2">
                                    @can('transaccion.view')
                                    ...
                                    @endcan
                                    @can('transaccion.delete')
                                        <button wire:click="delete({{ $transaccion->id }})" wire:confirm="¿Estás seguro que deseas eliminar esta transaccion?"
